...
\acknowledgements{
    I'm extremely grateful to my parents for always supporting me and to my sister for showing me 
    the ropes when I moved to Prague.
    I would also like to extend my deepest gratitude to my supervisor doc. Ing. Ivan Šimeček, Ph.D., for providing invaluable guidance
...
Since MMEF uses the coordinate format to store sparse matrices, it may omit elements from symmetrical matrices, that can be inferred
from their symmetrical counterparts which can result in significant on-disk size reduction. Matrices 2, 3 and 4 in Figure~\ref{impl:stor:size}
...
Section~\ref{csr5:spmv} established that a tile may share at most one output element with its neighbor,
...
\dirtree{%
    ...
    .1 src\DTcomment{the directory of source code}.
    .2 include\DTcomment{Header files}.
    .2 lib\DTcomment{Libraries}.
    .2 apps\DTcomment{Applications}.
    .2 benchmarks\DTcomment{Benchmarks}.
    .2 docs/paper\DTcomment{the thesis source in \LaTeX format}.
