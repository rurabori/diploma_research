...
\abstractCS{Cílem tehle práce bylo prozkoumat možnosti distribuovaného násobení řídké matice vektorem nekolika 
procesy, s použitím MPI a CSR5. Výsledkem tohto výzkumu je C++ knižnice \texttt{dim}, která poskytuje potřebné 
stavební . Potencionálni zrychlení 
distribuovaného násobení řídké matice vektorem pak bylo meřeno na implementaci metody konjugovaných gradientů,
a porovnávano s jedno-procesovou implementací založenou na CSR5 i distribuovanou implementací pomocí PETSc.}
...
\subsection{Matrix Market Exchange Format}\label{theory:MMEF}
...
The implementation will be reffered to as \csre{dim}.
...
degree of paralelism and small critical sections. For this reason \csre{dim}, stores all matrices in
...
Tile pointer array contains the index of the first element' row. It works similarly to a CSR
...
Amids the new additions to C++20 are bit-manipulation functions in \csre{<bit>} header.
...
struct descriptor {
    ...
    \end{minted}
    \caption{Tile descriptor pseudo-code.}
\end{figure}
...
\begin{figure}[htp]
    ...
    \begin{tikzpicture}
        ...
        \path (1, -0.5) -- (1, -1) node [black, font=\Huge, midway, sloped] {$\dots$};

    \end{tikzpicture}
    \caption{Processing a single tile}
\end{figure}
...
are large. This section compares Matrix Market Exchange Format~\cite{mmef} introduced in \ref{theory:MMEF}
and HDF5 backed format
introduced by Matlab 6.0 for storing sparse matrices, comparing both on-disk size and throughput
...
\begin{algorithm}
    ...
    \begin{algorithmic}
        ...
        \State $\var{tile\_desc} \gets \var{datasets["tile\_desc"][first:first + count]}$
        \State $\var{tile\_ptr} \gets \var{datasets["tile\_ptr"][first:first + count + 1]}$
        \State $\var{empty\_off\_ptr} \gets \var{datasets["empty\_offset\_ptr"][first:first+count+1]}$
        \State $\var{empty\_off} \gets \var{datasets["empty\_offset"][empty\_off\_ptr[0]:empty\_off\_ptr[-1]]}$
        \EndFunction
    \end{algorithmic}
\end{algorithm}
...
mechanism is explained in Section~\ref{dspmv:sync}. If all processes need the result vector,
...
\subsection{Synchronizing overlapping output ranges}\label{dspmv:sync}
...
\begin{figure}[htp]
    ...
    \end{tikzpicture}
    \caption{Distribution of result of \(Ax = y\) assuming \(x=\mathbb{1}\)}
\end{figure}
...
implementation of conjugate gradient solver using \csre{PETSc} (v3.15.1) was created.
...
(PETSc \csre{MatType = MATMPIAIJ}) was used for benchmarking.
