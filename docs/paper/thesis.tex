...
% TODO
This chapter introduces the technologies used in implementation of this thesis.
...
In C++ there are several ways to achieve vectorization\footnote{Note that for compiler to enable
    vector instructions, target architecture must be provided.}.
...
To enable OpenMP, compiler must be invoked with a \csre{-fopenmp}
...
team. This means the execution is only parallelized if the loop is performed inside of a
\texttt{parallel} region in first place.
...
Meaning when thread finishes processing its assigned chunk (of \texttt{chunk\_size} elements),
...
round-robin fashion. This is used heavily in the computational kernel described in Section~\ref{csr5:spmv}.
...
and semaphores and can be also used to vectorize loops using the \texttt{simd} construct as as shown
...
As shown in Algorithm~\ref{coo:spmv}, to perform SpMV with a matrix stored in COO format, it is necessary
to iterate over all non-zero elements, extracting their value, column index and row index from the
relevant arrays in the COO structure. Then performing the multiplication \(\var{value * x[col]}\) and
finally add the sub-result to \(\var{y[row]}\). This algorithm is not well suited for parallelization,
as all the additions to the result vector \(\var{y}\) would have to be performed atomically. It may also
degrade single-thread performance as order of non-zero elements is not specified, meaning accesses to $\var{x}$
...
\begin{algorithm}
    ...
    \begin{algorithmic}
        ...
        \State $\var{y[row]} \pluseq \var{value * x[col]}$
        \EndFor
...
the non-zero elements are grouped by their rows, the outer loop of Algorithm~\ref{csr:spmv}
can be parallelized with no synchronization necessary (no overlapping outputs in \(\var{y}\)).
The issue with parallelized version of this algorithm is load-balancing as some rows may have
...
\begin{algorithm}
    ...
    \begin{algorithmic}
        ...
        \State $\var{row\_result} \pluseq \var{value * x[col]}$
        \EndFor
...
The algorithm for performing parallel SpMV using matrices stored in CSR5 format is outlined in
Section~\ref{csr5:spmv}.
...
With the \texttt{distributing s} step removed, the speedup of D-CSR5 based implementation compared to \csre{PETSc}
...
\begin{figure}[htp]
    ...
    \begin{tabular}{*{7}{c}}
        ...
        \bottomrule
    \end{tabular}
    \caption{Speedup of iteration and SpMV step of iteration using when using D-CSR5 vs PETSc}
\end{figure}
