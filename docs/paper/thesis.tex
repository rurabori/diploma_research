...
Matrix used for benchmarking: e40r500.mtx: 17281x17281 with 553956 non-zero elements 17.6MB on disk size (MM format). \\

Compiler: Clang 13.0.1 Ryzen 7 3700X\@3.7GHz, nvme SSD. \\
...
the header and matrix dimensions. % TODO: cite mmio
The entries themselves however, need to be parsed manually. Most implementations use \mintinline{C++}{scanf}.
Initial loading IO implementation in dim used scanf as well. This however proved to be a bottleneck.
Loading the benchmarked matrix took 215.818ms. After profiling, it beacame obvious that parsing 
the string representation was the issue as 97.89\% of time spent was in scanf itself.

To improve the loading times (1.1GB matrix took around a minute to load), in an effort to 
...
was deployed. While it provided around 1.2x speed up (matrix loading taking 176.424ms), it was still
far too slow. 
...
scnlib + mapped: 176.424ms, 97.17\% of time spent in scn::scan (basically scanf).
pagefaults are basically the same, IO doesn't have big impact, parsing does.
Scales horribly to actually big matrices, hard to paralelize.

HDF5: 2.854ms, ~100x speedup. 48\% of time spent in the dataset reading function. 16\% of time spent in \mintinline{C++}{vector::resize}.
Would benefit from non-initializing resize, current implementation has to 0 initialize the vectors, but those are dead writes because
the data is immediately loaded into memory.
