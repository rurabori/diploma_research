...
namespace detail {
    ...
    //! @brief this is just a workaround for tooling not recognizing std::span as template-template argument.
    template<typename Ty>
    using impl_span = std::span<Ty>;
} // namespace detail
...
class vec_view : public detail::vec_impl<ValueType, detail::impl_span>
{
    using parent_t = detail::vec_impl<ValueType, detail::impl_span>;

public:
    explicit vec_view(detail::impl_span<ValueType> vals) : parent_t(std::move(vals)) {}
...
template<typename ValueType>
class vec : public detail::vec_impl<ValueType, memory::cache_aligned_vector>
{
    using parent_t = detail::vec_impl<ValueType, memory::cache_aligned_vector>;
