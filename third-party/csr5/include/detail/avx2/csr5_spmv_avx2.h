...
inline void partition_fast_track(const vT* d_value_partition, const vT* d_x, const iT* d_column_index_partition,
                                 ...
                                 const iT start_row_start, const vT alpha, const int sigma, const int stride_vT,
                                 const bool direct) {
...
void spmv_csr5_compute_kernel(const iT* d_column_index, const vT* d_value, const iT* d_row_pointer, const vT* d_x,
                              ...
                              const int bit_scansum_offset, const vT alpha, const int c_sigma) {
        ...
        for (int par_id = 0; par_id < p - 1; par_id++) {
            ...
            if (row_start == row_stop) // fast track through reduction
            {
                ...
                partition_fast_track<iT, vT>(d_value_partition, d_x, d_column_index_partition, d_calibrator, d_y,
                                             row_start, par_id, tid, start_row_start, alpha, c_sigma, stride_vT,
                                             fast_direct);
            } else // normal track for all the other partitions
            {
                ...
                d_partition_descriptor128i
                  = (__m128i*)&d_partition_descriptor[par_id * ANONYMOUSLIB_CSR5_OMEGA * num_packet];
                ...
                _mm256_store_si256((__m256i*)std::data(s_cond), local_bit256i);
                if (s_cond[0]) first_direct = true;
                ...
                if (par_id == tid * chunk) first_all_direct = first_direct;
                    ...
                    if (!store_to_offchip) {
                        ...
                        first_sum256d = _mm256_add_pd(
                          _mm256_and_pd(_mm256_castsi256_pd(_mm256_cmpeq_epi64(tmp256i, _mm256_set1_epi64x(0))),
                                        first_sum256d),
                          _mm256_and_pd(
                            _mm256_castsi256_pd(_mm256_cmpeq_epi64(tmp256i, _mm256_set1_epi64x(0xFFFFFFFFFFFFFFFF))),
                            sum256d));
                ...
                sum256d = _mm256_and_pd(_mm256_castsi256_pd(_mm256_setr_epi64x(0xFFFFFFFFFFFFFFFF, 0xFFFFFFFFFFFFFFFF,
...
void spmv_csr5_tail_partition_kernel(const iT* d_row_pointer, const iT* d_column_index, const vT* d_value,
                                     ...
                                     const int sigma, const vT alpha) {
...
#pragma omp parallel for
    for (iT row_id = tail_partition_start; row_id < m; row_id++) {
        ...
        vT sum = 0;
        for (iT idx = idx_start; idx < idx_stop; idx++) sum += d_value[idx] * d_x[d_column_index[idx]]; // * alpha;
...
int csr5_spmv(const int sigma, const ANONYMOUSLIB_IT p, const ANONYMOUSLIB_IT m, const int bit_y_offset,
    ...
    spmv_csr5_compute_kernel<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, ANONYMOUSLIB_VT>(
      column_index, value, row_pointer, x, partition_pointer, partition_descriptor, partition_descriptor_offset_pointer,
      partition_descriptor_offset, calibrator, y, p, num_packet, bit_y_offset, bit_scansum_offset, alpha, sigma);
    ...
    spmv_csr5_tail_partition_kernel<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, ANONYMOUSLIB_VT>(
      row_pointer, column_index, value, x, y, tail_partition_start, p, m, sigma, alpha);
