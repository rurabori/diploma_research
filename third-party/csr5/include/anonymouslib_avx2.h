...
int anonymouslibHandle<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, ANONYMOUSLIB_VT>::asCSR() {
    ...
    if (_format == ANONYMOUSLIB_FORMAT_CSR) return err;

    if (_format == ANONYMOUSLIB_FORMAT_CSR5) {
        // convert csr5 data to csr data
        err = aosoa_transpose(_csr5_sigma, _nnz, _csr5_partition_pointer, _csr_column_index, _csr_value, false);
...
int anonymouslibHandle<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, ANONYMOUSLIB_VT>::asCSR5() {
    ...
    if (_format == ANONYMOUSLIB_FORMAT_CSR5) return err;
    ...
    if (_format == ANONYMOUSLIB_FORMAT_CSR) {
        ...
        _csr5_partition_pointer
          = (ANONYMOUSLIB_UIT*)_mm_malloc((_p + 1) * sizeof(ANONYMOUSLIB_UIT), ANONYMOUSLIB_X86_CACHELINE);
        ...
        _temp_calibrator
          = (ANONYMOUSLIB_VT*)_mm_malloc(num_thread * ANONYMOUSLIB_X86_CACHELINE, ANONYMOUSLIB_X86_CACHELINE);
        memset(_temp_calibrator, 0, num_thread * ANONYMOUSLIB_X86_CACHELINE);

        _csr5_partition_descriptor_offset_pointer
          = (ANONYMOUSLIB_IT*)_mm_malloc((_p + 1) * sizeof(ANONYMOUSLIB_IT), ANONYMOUSLIB_X86_CACHELINE);
        ...
        if (err != ANONYMOUSLIB_SUCCESS) return ANONYMOUSLIB_CSR_TO_CSR5_FAILED;
        ...
        err = generate_partition_descriptor<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT>(
          ...
          _csr5_partition_pointer, _csr5_partition_descriptor, _csr5_partition_descriptor_offset_pointer,
          &_num_offsets);
        if (err != ANONYMOUSLIB_SUCCESS) return ANONYMOUSLIB_CSR_TO_CSR5_FAILED;
        ...
        if (_num_offsets) {
            ...
            _csr5_partition_descriptor_offset
              = (ANONYMOUSLIB_IT*)_mm_malloc(_num_offsets * sizeof(ANONYMOUSLIB_IT), ANONYMOUSLIB_X86_CACHELINE);
        ...
        err = aosoa_transpose(_csr5_sigma, _nnz, _csr5_partition_pointer, _csr_column_index, _csr_value, true);
        if (err != ANONYMOUSLIB_SUCCESS) return ANONYMOUSLIB_CSR_TO_CSR5_FAILED;
...
int anonymouslibHandle<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, ANONYMOUSLIB_VT>::spmv(const ANONYMOUSLIB_VT alpha,
    ...
    if (_format == ANONYMOUSLIB_FORMAT_CSR5) {
        csr5_spmv<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, ANONYMOUSLIB_VT>(
          ...
          _csr_value, _csr5_partition_pointer, _csr5_partition_descriptor, _csr5_partition_descriptor_offset_pointer,
          _csr5_partition_descriptor_offset, _temp_calibrator, _tail_partition_start, alpha, _x, y);
    }
