...
__inline__ __device__ vT candidate(const vT* d_value_partition, const vT* d_x, cudaTextureObject_t d_x_tex,
                                   const iT* d_column_index_partition, const iT candidate_index, const vT alpha) {
    vT x = 0;
...
__forceinline__ __device__ vT segmented_sum_shfl(vT tmp_sum, const int scansum_offset, const int lane_id) {
...
template<typename vT>
__forceinline__ __device__ vT segmented_sum(vT tmp_sum, volatile vT* s_sum, const int scansum_offset,
                                            const int lane_id) {
    if (lane_id) s_sum[lane_id - 1] = tmp_sum;
...
__inline__ __device__ void partition_fast_track(const vT* d_value_partition, const vT* d_x, cudaTextureObject_t d_x_tex,
                                                const iT* d_column_index_partition, vT* d_calibrator,
#if __CUDA_ARCH__ < 300
                                                volatile vT* s_sum,
#endif
                                                const int lane_id, const iT par_id, const vT alpha) {
    vT sum = 0;

#pragma unroll
    for (int i = 0; i < c_sigma; i++)
        sum += candidate<iT, vT>(d_value_partition, d_x, d_x_tex, d_column_index_partition,
                                 i * ANONYMOUSLIB_CSR5_OMEGA + lane_id, alpha);

#if __CUDA_ARCH__ >= 300 // use shfl intrinsic
    sum = sum_32_shfl<vT>(sum);
    if (!lane_id) d_calibrator[par_id] = sum;
#else // use smem
    s_sum[lane_id] = sum;
    sum_32<vT>(s_sum, lane_id);
    if (!lane_id) d_calibrator[par_id] = s_sum[0];
#endif
}
...
#pragma unroll
    ...
    if (direct) d_y[empty_rows ? d_partition_descriptor_offset[offset_pointer + y_offset] : y_offset] = last_sum;

    // the first/last value of the first thread goes to calibration
    if (!lane_id) d_calibrator[par_id] = direct ? first_sum : last_sum;
}

template<typename iT, typename uiT, typename vT, int c_sigma>
__inline__ __device__ void
  spmv_partition(const iT* d_column_index_partition, const vT* d_value_partition, const iT* d_row_pointer,
                 const vT* d_x, cudaTextureObject_t d_x_tex, const uiT* d_partition_pointer,
...
template<typename iT, typename uiT, typename vT, int c_sigma>
__global__ void
  spmv_csr5_compute_kernel(const iT* d_column_index, const vT* d_value, const iT* d_row_pointer, const vT* d_x,
                           cudaTextureObject_t d_x_tex, const uiT* d_partition_pointer,
                           const uiT* d_partition_descriptor, const iT* d_partition_descriptor_offset_pointer,
                           const iT* d_partition_descriptor_offset, vT* d_calibrator, vT* d_y, const iT p,
                           const int num_packet, const int bit_y_offset, const int bit_scansum_offset, const vT alpha) {
    // warp lane id
    ...
    spmv_partition<iT, uiT, vT, c_sigma>(
      &d_column_index[par_id * ANONYMOUSLIB_CSR5_OMEGA * c_sigma], &d_value[par_id * ANONYMOUSLIB_CSR5_OMEGA * c_sigma],
      d_row_pointer, d_x, d_x_tex, d_partition_pointer,
      &d_partition_descriptor[par_id * ANONYMOUSLIB_CSR5_OMEGA * num_packet], d_partition_descriptor_offset_pointer,
      d_partition_descriptor_offset, d_calibrator, d_y, par_id, lane_id, bunch_id, bit_y_offset, bit_scansum_offset,
      alpha);
}

template<typename iT, typename uiT, typename vT>
__global__ void spmv_csr5_calibrate_kernel(const uiT* d_partition_pointer, const vT* d_calibrator, vT* d_y,
                                           const iT p) {
    ...
    if (s_partition_pointer[0] == s_partition_pointer[ANONYMOUSLIB_THREAD_GROUP - 1]) {
        ...
        if (!local_id) atomicAdd(&d_y[s_partition_pointer[0]], sum);
    ...
    if (global_id < p - 1) {
        ...
        if (row_start_previous != row_start_current) {
            ...
            if (row_start_target == s_partition_pointer[0]
                || row_start_target == s_partition_pointer[ANONYMOUSLIB_THREAD_GROUP - 1])
...
template<typename iT, typename uiT, typename vT>
__global__ void spmv_csr5_tail_partition_kernel(const iT* d_row_pointer, const iT* d_column_index, const vT* d_value,
                                                const vT* d_x, cudaTextureObject_t d_x_tex, vT* d_y,
                                                const iT tail_partition_start, const iT p, const int sigma,
                                                const vT alpha) {
    const int local_id = threadIdx.x;
    ...
    if (!local_id) d_y[row_id] = !blockIdx.x ? d_y[row_id] + sum : sum;
}
...
int csr5_spmv(const int sigma, const ANONYMOUSLIB_IT p, const ANONYMOUSLIB_IT m, const int bit_y_offset,
              ...
              const ANONYMOUSLIB_IT tail_partition_start, const ANONYMOUSLIB_VT alpha, const ANONYMOUSLIB_VT* x,
              cudaTextureObject_t x_tex, ANONYMOUSLIB_VT* y) {
    ...
    switch (sigma) {
        case 4:
            spmv_csr5_compute_kernel<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, ANONYMOUSLIB_VT, 4>
              <<<num_blocks, num_threads>>>(column_index, value, row_pointer, x, x_tex, partition_pointer,
                                            ...
                                            partition_descriptor_offset, calibrator, y, p, num_packet, bit_y_offset,
                                            bit_scansum_offset, alpha);
            break;
        case 5:
            spmv_csr5_compute_kernel<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, ANONYMOUSLIB_VT, 5>
              <<<num_blocks, num_threads>>>(column_index, value, row_pointer, x, x_tex, partition_pointer,
                                            partition_descriptor, partition_descriptor_offset_pointer,
                                            partition_descriptor_offset, calibrator, y, p, num_packet, bit_y_offset,
                                            bit_scansum_offset, alpha);
            break;
        case 6:
            spmv_csr5_compute_kernel<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, ANONYMOUSLIB_VT, 6>
              <<<num_blocks, num_threads>>>(column_index, value, row_pointer, x, x_tex, partition_pointer,
                                            partition_descriptor, partition_descriptor_offset_pointer,
                                            partition_descriptor_offset, calibrator, y, p, num_packet, bit_y_offset,
                                            bit_scansum_offset, alpha);
            break;
        case 7:
            spmv_csr5_compute_kernel<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, ANONYMOUSLIB_VT, 7>
              <<<num_blocks, num_threads>>>(column_index, value, row_pointer, x, x_tex, partition_pointer,
                                            partition_descriptor, partition_descriptor_offset_pointer,
                                            partition_descriptor_offset, calibrator, y, p, num_packet, bit_y_offset,
                                            bit_scansum_offset, alpha);
            break;
        case 8:
            spmv_csr5_compute_kernel<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, ANONYMOUSLIB_VT, 8>
              <<<num_blocks, num_threads>>>(column_index, value, row_pointer, x, x_tex, partition_pointer,
                                            partition_descriptor, partition_descriptor_offset_pointer,
                                            partition_descriptor_offset, calibrator, y, p, num_packet, bit_y_offset,
                                            bit_scansum_offset, alpha);
            break;
        case 9:
            spmv_csr5_compute_kernel<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, ANONYMOUSLIB_VT, 9>
              <<<num_blocks, num_threads>>>(column_index, value, row_pointer, x, x_tex, partition_pointer,
                                            partition_descriptor, partition_descriptor_offset_pointer,
                                            partition_descriptor_offset, calibrator, y, p, num_packet, bit_y_offset,
                                            bit_scansum_offset, alpha);
            break;
        case 10:
            spmv_csr5_compute_kernel<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, ANONYMOUSLIB_VT, 10>
              <<<num_blocks, num_threads>>>(column_index, value, row_pointer, x, x_tex, partition_pointer,
                                            partition_descriptor, partition_descriptor_offset_pointer,
                                            partition_descriptor_offset, calibrator, y, p, num_packet, bit_y_offset,
                                            bit_scansum_offset, alpha);
        ...
        case 11:
            spmv_csr5_compute_kernel<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, ANONYMOUSLIB_VT, 11>
              <<<num_blocks, num_threads>>>(column_index, value, row_pointer, x, x_tex, partition_pointer,
                                            partition_descriptor, partition_descriptor_offset_pointer,
                                            partition_descriptor_offset, calibrator, y, p, num_packet, bit_y_offset,
                                            bit_scansum_offset, alpha);
            break;
        case 12:
            spmv_csr5_compute_kernel<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, ANONYMOUSLIB_VT, 12>
              <<<num_blocks, num_threads>>>(column_index, value, row_pointer, x, x_tex, partition_pointer,
                                            partition_descriptor, partition_descriptor_offset_pointer,
                                            partition_descriptor_offset, calibrator, y, p, num_packet, bit_y_offset,
                                            bit_scansum_offset, alpha);
            break;
        case 13:
            spmv_csr5_compute_kernel<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, ANONYMOUSLIB_VT, 13>
              <<<num_blocks, num_threads>>>(column_index, value, row_pointer, x, x_tex, partition_pointer,
                                            partition_descriptor, partition_descriptor_offset_pointer,
                                            partition_descriptor_offset, calibrator, y, p, num_packet, bit_y_offset,
                                            bit_scansum_offset, alpha);
            break;
        case 14:
            spmv_csr5_compute_kernel<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, ANONYMOUSLIB_VT, 14>
              <<<num_blocks, num_threads>>>(column_index, value, row_pointer, x, x_tex, partition_pointer,
                                            partition_descriptor, partition_descriptor_offset_pointer,
                                            partition_descriptor_offset, calibrator, y, p, num_packet, bit_y_offset,
                                            bit_scansum_offset, alpha);
            break;
        case 15:
            spmv_csr5_compute_kernel<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, ANONYMOUSLIB_VT, 15>
              <<<num_blocks, num_threads>>>(column_index, value, row_pointer, x, x_tex, partition_pointer,
                                            partition_descriptor, partition_descriptor_offset_pointer,
                                            partition_descriptor_offset, calibrator, y, p, num_packet, bit_y_offset,
                                            bit_scansum_offset, alpha);
            break;
        case 16:
            spmv_csr5_compute_kernel<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, ANONYMOUSLIB_VT, 16>
              <<<num_blocks, num_threads>>>(column_index, value, row_pointer, x, x_tex, partition_pointer,
                                            partition_descriptor, partition_descriptor_offset_pointer,
                                            partition_descriptor_offset, calibrator, y, p, num_packet, bit_y_offset,
                                            bit_scansum_offset, alpha);
        ...
        case 17:
            ...
            break;
        case 18:
            spmv_csr5_compute_kernel<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, ANONYMOUSLIB_VT, 18>
              <<<num_blocks, num_threads>>>(column_index, value, row_pointer, x, x_tex, partition_pointer,
                                            partition_descriptor, partition_descriptor_offset_pointer,
                                            partition_descriptor_offset, calibrator, y, p, num_packet, bit_y_offset,
                                            bit_scansum_offset, alpha);
            break;
        case 19:
            spmv_csr5_compute_kernel<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, ANONYMOUSLIB_VT, 19>
              <<<num_blocks, num_threads>>>(column_index, value, row_pointer, x, x_tex, partition_pointer,
                                            partition_descriptor, partition_descriptor_offset_pointer,
                                            partition_descriptor_offset, calibrator, y, p, num_packet, bit_y_offset,
                                            bit_scansum_offset, alpha);
            break;
        case 20:
            spmv_csr5_compute_kernel<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, ANONYMOUSLIB_VT, 20>
              <<<num_blocks, num_threads>>>(column_index, value, row_pointer, x, x_tex, partition_pointer,
                                            partition_descriptor, partition_descriptor_offset_pointer,
                                            partition_descriptor_offset, calibrator, y, p, num_packet, bit_y_offset,
                                            bit_scansum_offset, alpha);
        ...
        case 21:
            spmv_csr5_compute_kernel<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, ANONYMOUSLIB_VT, 21>
              <<<num_blocks, num_threads>>>(column_index, value, row_pointer, x, x_tex, partition_pointer,
                                            partition_descriptor, partition_descriptor_offset_pointer,
                                            partition_descriptor_offset, calibrator, y, p, num_packet, bit_y_offset,
                                            bit_scansum_offset, alpha);
            break;
        case 22:
            spmv_csr5_compute_kernel<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, ANONYMOUSLIB_VT, 22>
              <<<num_blocks, num_threads>>>(column_index, value, row_pointer, x, x_tex, partition_pointer,
                                            partition_descriptor, partition_descriptor_offset_pointer,
                                            partition_descriptor_offset, calibrator, y, p, num_packet, bit_y_offset,
                                            bit_scansum_offset, alpha);
            break;
        case 23:
            spmv_csr5_compute_kernel<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, ANONYMOUSLIB_VT, 23>
              <<<num_blocks, num_threads>>>(column_index, value, row_pointer, x, x_tex, partition_pointer,
                                            partition_descriptor, partition_descriptor_offset_pointer,
                                            partition_descriptor_offset, calibrator, y, p, num_packet, bit_y_offset,
                                            bit_scansum_offset, alpha);
            break;
        case 24:
            spmv_csr5_compute_kernel<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, ANONYMOUSLIB_VT, 24>
              <<<num_blocks, num_threads>>>(column_index, value, row_pointer, x, x_tex, partition_pointer,
                                            partition_descriptor, partition_descriptor_offset_pointer,
                                            partition_descriptor_offset, calibrator, y, p, num_packet, bit_y_offset,
                                            bit_scansum_offset, alpha);
            break;
        case 25:
            spmv_csr5_compute_kernel<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, ANONYMOUSLIB_VT, 25>
              <<<num_blocks, num_threads>>>(column_index, value, row_pointer, x, x_tex, partition_pointer,
                                            partition_descriptor, partition_descriptor_offset_pointer,
                                            partition_descriptor_offset, calibrator, y, p, num_packet, bit_y_offset,
                                            bit_scansum_offset, alpha);
            break;
        case 26:
            spmv_csr5_compute_kernel<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, ANONYMOUSLIB_VT, 26>
              <<<num_blocks, num_threads>>>(column_index, value, row_pointer, x, x_tex, partition_pointer,
                                            partition_descriptor, partition_descriptor_offset_pointer,
                                            partition_descriptor_offset, calibrator, y, p, num_packet, bit_y_offset,
                                            bit_scansum_offset, alpha);
            break;
        case 27:
            spmv_csr5_compute_kernel<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, ANONYMOUSLIB_VT, 27>
              <<<num_blocks, num_threads>>>(column_index, value, row_pointer, x, x_tex, partition_pointer,
                                            partition_descriptor, partition_descriptor_offset_pointer,
                                            partition_descriptor_offset, calibrator, y, p, num_packet, bit_y_offset,
                                            bit_scansum_offset, alpha);
            break;
        case 28:
            spmv_csr5_compute_kernel<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, ANONYMOUSLIB_VT, 28>
              <<<num_blocks, num_threads>>>(column_index, value, row_pointer, x, x_tex, partition_pointer,
                                            partition_descriptor, partition_descriptor_offset_pointer,
                                            partition_descriptor_offset, calibrator, y, p, num_packet, bit_y_offset,
                                            bit_scansum_offset, alpha);
            break;
        case 29:
            spmv_csr5_compute_kernel<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, ANONYMOUSLIB_VT, 29>
              <<<num_blocks, num_threads>>>(column_index, value, row_pointer, x, x_tex, partition_pointer,
                                            partition_descriptor, partition_descriptor_offset_pointer,
                                            partition_descriptor_offset, calibrator, y, p, num_packet, bit_y_offset,
                                            bit_scansum_offset, alpha);
            break;
        case 30:
            spmv_csr5_compute_kernel<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, ANONYMOUSLIB_VT, 30>
              <<<num_blocks, num_threads>>>(column_index, value, row_pointer, x, x_tex, partition_pointer,
                                            partition_descriptor, partition_descriptor_offset_pointer,
                                            partition_descriptor_offset, calibrator, y, p, num_packet, bit_y_offset,
                                            bit_scansum_offset, alpha);
        ...
        case 31:
            spmv_csr5_compute_kernel<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, ANONYMOUSLIB_VT, 31>
              <<<num_blocks, num_threads>>>(column_index, value, row_pointer, x, x_tex, partition_pointer,
                                            partition_descriptor, partition_descriptor_offset_pointer,
                                            partition_descriptor_offset, calibrator, y, p, num_packet, bit_y_offset,
                                            bit_scansum_offset, alpha);
            break;
        case 32:
            spmv_csr5_compute_kernel<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, ANONYMOUSLIB_VT, 32>
              <<<num_blocks, num_threads>>>(column_index, value, row_pointer, x, x_tex, partition_pointer,
                                            partition_descriptor, partition_descriptor_offset_pointer,
                                            partition_descriptor_offset, calibrator, y, p, num_packet, bit_y_offset,
                                            bit_scansum_offset, alpha);
    ...
    spmv_csr5_calibrate_kernel<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, ANONYMOUSLIB_VT>
      <<<num_blocks, num_threads>>>(partition_pointer, calibrator, y, p);

    num_threads = ANONYMOUSLIB_CSR5_OMEGA;
    num_blocks = m - tail_partition_start;

    spmv_csr5_tail_partition_kernel<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, ANONYMOUSLIB_VT><<<num_blocks, num_threads>>>(
      row_pointer, column_index, value, x, x_tex, y, tail_partition_start, p, sigma, alpha);
