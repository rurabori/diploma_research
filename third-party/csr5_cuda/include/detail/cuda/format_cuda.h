...
int format_warmup() {
    ...
    for (int i = 0; i < 50; i++) warmup_kernel<<<num_blocks, num_threads>>>(d_scan);

    cudaFree(d_scan);
}

template<typename iT, typename uiT>
__global__ void generate_partition_pointer_s1_kernel(const iT* d_row_pointer, uiT* d_partition_pointer, const int sigma,
...
template<typename iT, typename uiT>
__global__ void generate_partition_pointer_s2_kernel(const iT* d_row_pointer, uiT* d_partition_pointer) {
    ...
    for (int i = 0; i < loop; i++) {
        ...
        if (row_idx < stop) {
            ...
            if (row_off_l == row_off_r) s_dirty[0] = 1;
        }
        __syncthreads();
...
int generate_partition_pointer(const int sigma, const ANONYMOUSLIB_IT p, const ANONYMOUSLIB_IT m,
    ...
    //    checkCudaErrors(cudaMemcpy(check_partition_pointer, partition_pointer, (p+1) * sizeof(ANONYMOUSLIB_UIT),
...
template<typename iT, typename uiT>
__global__ void generate_partition_descriptor_s1_kernel(const iT* d_row_pointer, uiT* d_partition_descriptor,
                                                        const iT m, const int sigma, const int bit_all_offset,
...
template<typename iT, typename uiT>
__global__ void generate_partition_descriptor_s2_kernel(const uiT* d_partition_pointer, uiT* d_partition_descriptor,
                                                        iT* d_partition_descriptor_offset_pointer, const int sigma,
                                                        const int num_packet, const int bit_y_offset,
                                                        const int bit_scansum_offset, const int p) {
    ...
    volatile __shared__ int
      s_segn_scan[(ANONYMOUSLIB_CSR5_OMEGA + 1) * ANONYMOUSLIB_THREAD_GROUP / ANONYMOUSLIB_CSR5_OMEGA];
#endif
    volatile __shared__ int
      s_present[(ANONYMOUSLIB_CSR5_OMEGA + 1) * ANONYMOUSLIB_THREAD_GROUP / ANONYMOUSLIB_CSR5_OMEGA];
    ...
    if (!lane_id && with_empty_rows) {
        d_partition_descriptor_offset_pointer[par_id]
          = s_segn_scan[bunch_id * (ANONYMOUSLIB_CSR5_OMEGA + 1)
                        + ANONYMOUSLIB_CSR5_OMEGA];   // the total number of segments in this partition
        d_partition_descriptor_offset_pointer[p] = 1; // the total number of segments in total
    }
...
__global__ void generate_partition_descriptor_s3_kernel(iT* d_partition_descriptor_offset_pointer, const int p) {
    const int local_id = threadIdx.x;
    ...
    for (int i = 0; i < loop; i++) {
        s_partition_descriptor_offset_pointer[local_id]
          = (local_id + i * local_size < p) ? d_partition_descriptor_offset_pointer[local_id + i * local_size] : 0;
        __syncthreads();
        ...
        if (!local_id) s_partition_descriptor_offset_pointer[256] += sum;
        __syncthreads();

        sum = s_partition_descriptor_offset_pointer[256];

        if (local_id + i * local_size < p + 1)
            d_partition_descriptor_offset_pointer[local_id + i * local_size]
              = s_partition_descriptor_offset_pointer[local_id];
    }
}
...
int generate_partition_descriptor(const int sigma, const ANONYMOUSLIB_IT p, const ANONYMOUSLIB_IT m,
    ...
    generate_partition_descriptor_s1_kernel<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT>
      <<<num_blocks, num_threads>>>(row_pointer, partition_descriptor, m, sigma, bit_all_offset, num_packet);
    ...
    generate_partition_descriptor_s2_kernel<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT>
      <<<num_blocks, num_threads>>>(partition_pointer, partition_descriptor, partition_descriptor_offset_pointer, sigma,
                                    num_packet, bit_y_offset, bit_scansum_offset, p);

    ANONYMOUSLIB_IT num_offsets;
    checkCudaErrors(cudaMemcpy(&num_offsets, &partition_descriptor_offset_pointer[p], sizeof(ANONYMOUSLIB_IT),
                               cudaMemcpyDeviceToHost));
    ...
    *_num_offsets = num_offsets;
    ...
    //    // print for debug
    //    ANONYMOUSLIB_UIT *check_partition_descriptor = (ANONYMOUSLIB_UIT
    //    *)malloc(p*ANONYMOUSLIB_CSR5_OMEGA*num_packet*sizeof(ANONYMOUSLIB_UIT));
    //    checkCudaErrors(cudaMemcpy(check_partition_descriptor, partition_descriptor,
    //    p*ANONYMOUSLIB_CSR5_OMEGA*num_packet * sizeof(ANONYMOUSLIB_UIT), cudaMemcpyDeviceToHost));
    ...
    //    print_tile<ANONYMOUSLIB_UIT>(&check_partition_descriptor[num_packet * ANONYMOUSLIB_CSR5_OMEGA], num_packet,
...
__inline__ __device__ void
  partition_normal_track_empty_Ologn(const iT* d_row_pointer, const uiT* d_partition_descriptor,
                                     const iT* d_partition_descriptor_offset_pointer, iT* d_partition_descriptor_offset,
                                     const iT par_id, const int lane_id, const int bit_y_offset,
                                     const int bit_scansum_offset, iT row_start, const iT row_stop, const int c_sigma) {
    bool local_bit;
    ...
    if (local_bit && lane_id) {
        ...
        const iT y_index
          = binary_search_right_boundary_kernel<iT>(&d_row_pointer[row_start + 1], idx, row_stop - row_start) - 1;
...
template<typename iT, typename uiT>
__inline__ __device__ void generate_partition_descriptor_offset_partition(
  const iT* d_row_pointer, const uiT* d_partition_pointer, const uiT* d_partition_descriptor,
  const iT* d_partition_descriptor_offset_pointer, iT* d_partition_descriptor_offset, const iT par_id,
  const int lane_id, const int bunch_id, const int bit_y_offset, const int bit_scansum_offset, const int c_sigma) {
    ...
    if (row_start >> 31) // with empty rows
    {
        ...
        partition_normal_track_empty_Ologn<iT, uiT>(
          d_row_pointer, d_partition_descriptor, d_partition_descriptor_offset_pointer, d_partition_descriptor_offset,
          par_id, lane_id, bit_y_offset, bit_scansum_offset, row_start, row_stop, c_sigma);
...
template<typename iT, typename uiT>
__global__ void generate_partition_descriptor_offset_kernel(const iT* d_row_pointer, const uiT* d_partition_pointer,
                                                            const uiT* d_partition_descriptor,
                                                            const iT* d_partition_descriptor_offset_pointer,
                                                            iT* d_partition_descriptor_offset, const iT p,
                                                            const int num_packet, const int bit_y_offset,
    ...
    if (par_id >= p - 1) return;

    generate_partition_descriptor_offset_partition<iT, uiT>(
      d_row_pointer, d_partition_pointer, &d_partition_descriptor[par_id * ANONYMOUSLIB_CSR5_OMEGA * num_packet],
      d_partition_descriptor_offset_pointer, d_partition_descriptor_offset, par_id, lane_id, bunch_id, bit_y_offset,
      bit_scansum_offset, c_sigma);
}
...
int generate_partition_descriptor_offset(const int sigma, const ANONYMOUSLIB_IT p, const int bit_y_offset,
    ...
    generate_partition_descriptor_offset_kernel<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT><<<num_blocks, num_threads>>>(
...
__global__ void aosoa_transpose_kernel_smem(T* d_data, const uiT* d_partition_pointer,
                                            const bool R2C) // R2C==true means CSR->CSR5, otherwise CSR5->CSR
{
    ...
    if (local_id < 2) s_par[local_id] = d_partition_pointer[blockIdx.x + local_id];
    __syncthreads();

    // if this is fast track partition, do not transpose it
    if (s_par[0] == s_par[1]) return;
...
#pragma unroll
    for (int idx = local_id; idx < ANONYMOUSLIB_CSR5_OMEGA * CSR5_SIGMA; idx += blockDim.x) {
        ...
        s_data[idx_y * (ANONYMOUSLIB_CSR5_OMEGA + 1) + idx_x]
          = d_data[blockIdx.x * ANONYMOUSLIB_CSR5_OMEGA * CSR5_SIGMA + idx];
    }
    __syncthreads();
...
#pragma unroll
    for (int idx = local_id; idx < ANONYMOUSLIB_CSR5_OMEGA * CSR5_SIGMA; idx += blockDim.x) {
        ...
        //    printf("Round %d\t thread %d, szT = %d, val = %f\n", idx, threadIdx.x, sizeof(T), s_data[idx_y *
        //    (ANONYMOUSLIB_CSR5_OMEGA+1) + idx_x]);
    }
}
...
int aosoa_transpose(const int sigma, const int nnz, const ANONYMOUSLIB_UIT* partition_pointer,
    ...
    switch (sigma) {
        case 4:
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, 4>
              <<<num_blocks, num_threads>>>(column_index, partition_pointer, R2C);
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_VT, ANONYMOUSLIB_UIT, 4>
              <<<num_blocks, num_threads>>>(value, partition_pointer, R2C);
            break;
        case 5:
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, 5>
              <<<num_blocks, num_threads>>>(column_index, partition_pointer, R2C);
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_VT, ANONYMOUSLIB_UIT, 5>
              <<<num_blocks, num_threads>>>(value, partition_pointer, R2C);
            break;
        case 6:
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, 6>
              <<<num_blocks, num_threads>>>(column_index, partition_pointer, R2C);
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_VT, ANONYMOUSLIB_UIT, 6>
              <<<num_blocks, num_threads>>>(value, partition_pointer, R2C);
            break;
        case 7:
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, 7>
              <<<num_blocks, num_threads>>>(column_index, partition_pointer, R2C);
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_VT, ANONYMOUSLIB_UIT, 7>
              <<<num_blocks, num_threads>>>(value, partition_pointer, R2C);
            break;
        case 8:
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, 8>
              <<<num_blocks, num_threads>>>(column_index, partition_pointer, R2C);
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_VT, ANONYMOUSLIB_UIT, 8>
              <<<num_blocks, num_threads>>>(value, partition_pointer, R2C);
            break;
        case 9:
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, 9>
              <<<num_blocks, num_threads>>>(column_index, partition_pointer, R2C);
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_VT, ANONYMOUSLIB_UIT, 9>
              <<<num_blocks, num_threads>>>(value, partition_pointer, R2C);
            break;
        case 10:
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, 10>
              <<<num_blocks, num_threads>>>(column_index, partition_pointer, R2C);
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_VT, ANONYMOUSLIB_UIT, 10>
              <<<num_blocks, num_threads>>>(value, partition_pointer, R2C);
            break;

        case 11:
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, 11>
              <<<num_blocks, num_threads>>>(column_index, partition_pointer, R2C);
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_VT, ANONYMOUSLIB_UIT, 11>
              <<<num_blocks, num_threads>>>(value, partition_pointer, R2C);
            break;
        case 12:
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, 12>
              <<<num_blocks, num_threads>>>(column_index, partition_pointer, R2C);
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_VT, ANONYMOUSLIB_UIT, 12>
              <<<num_blocks, num_threads>>>(value, partition_pointer, R2C);
            break;
        case 13:
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, 13>
              <<<num_blocks, num_threads>>>(column_index, partition_pointer, R2C);
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_VT, ANONYMOUSLIB_UIT, 13>
              <<<num_blocks, num_threads>>>(value, partition_pointer, R2C);
            break;
        case 14:
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, 14>
              <<<num_blocks, num_threads>>>(column_index, partition_pointer, R2C);
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_VT, ANONYMOUSLIB_UIT, 14>
              <<<num_blocks, num_threads>>>(value, partition_pointer, R2C);
            break;
        case 15:
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, 15>
              <<<num_blocks, num_threads>>>(column_index, partition_pointer, R2C);
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_VT, ANONYMOUSLIB_UIT, 15>
              <<<num_blocks, num_threads>>>(value, partition_pointer, R2C);
            break;
        case 16:
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, 16>
              <<<num_blocks, num_threads>>>(column_index, partition_pointer, R2C);
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_VT, ANONYMOUSLIB_UIT, 16>
              <<<num_blocks, num_threads>>>(value, partition_pointer, R2C);
            break;
        case 17:
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, 17>
              <<<num_blocks, num_threads>>>(column_index, partition_pointer, R2C);
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_VT, ANONYMOUSLIB_UIT, 17>
              <<<num_blocks, num_threads>>>(value, partition_pointer, R2C);
            break;
        case 18:
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, 18>
              <<<num_blocks, num_threads>>>(column_index, partition_pointer, R2C);
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_VT, ANONYMOUSLIB_UIT, 18>
              <<<num_blocks, num_threads>>>(value, partition_pointer, R2C);
            break;
        case 19:
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, 19>
              <<<num_blocks, num_threads>>>(column_index, partition_pointer, R2C);
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_VT, ANONYMOUSLIB_UIT, 19>
              <<<num_blocks, num_threads>>>(value, partition_pointer, R2C);
            break;
        case 20:
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, 20>
              <<<num_blocks, num_threads>>>(column_index, partition_pointer, R2C);
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_VT, ANONYMOUSLIB_UIT, 20>
              <<<num_blocks, num_threads>>>(value, partition_pointer, R2C);
            break;

        case 21:
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, 21>
              <<<num_blocks, num_threads>>>(column_index, partition_pointer, R2C);
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_VT, ANONYMOUSLIB_UIT, 21>
              <<<num_blocks, num_threads>>>(value, partition_pointer, R2C);
            break;
        case 22:
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, 22>
              <<<num_blocks, num_threads>>>(column_index, partition_pointer, R2C);
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_VT, ANONYMOUSLIB_UIT, 22>
              <<<num_blocks, num_threads>>>(value, partition_pointer, R2C);
            break;
        case 23:
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, 23>
              <<<num_blocks, num_threads>>>(column_index, partition_pointer, R2C);
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_VT, ANONYMOUSLIB_UIT, 23>
              <<<num_blocks, num_threads>>>(value, partition_pointer, R2C);
            break;
        case 24:
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, 24>
              <<<num_blocks, num_threads>>>(column_index, partition_pointer, R2C);
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_VT, ANONYMOUSLIB_UIT, 24>
              <<<num_blocks, num_threads>>>(value, partition_pointer, R2C);
            break;
        case 25:
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, 25>
              <<<num_blocks, num_threads>>>(column_index, partition_pointer, R2C);
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_VT, ANONYMOUSLIB_UIT, 25>
              <<<num_blocks, num_threads>>>(value, partition_pointer, R2C);
            break;
        case 26:
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, 26>
              <<<num_blocks, num_threads>>>(column_index, partition_pointer, R2C);
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_VT, ANONYMOUSLIB_UIT, 26>
              <<<num_blocks, num_threads>>>(value, partition_pointer, R2C);
            break;
        case 27:
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, 27>
              <<<num_blocks, num_threads>>>(column_index, partition_pointer, R2C);
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_VT, ANONYMOUSLIB_UIT, 27>
              <<<num_blocks, num_threads>>>(value, partition_pointer, R2C);
            break;
        case 28:
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, 28>
              <<<num_blocks, num_threads>>>(column_index, partition_pointer, R2C);
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_VT, ANONYMOUSLIB_UIT, 28>
              <<<num_blocks, num_threads>>>(value, partition_pointer, R2C);
            break;
        case 29:
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, 29>
              <<<num_blocks, num_threads>>>(column_index, partition_pointer, R2C);
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_VT, ANONYMOUSLIB_UIT, 29>
              <<<num_blocks, num_threads>>>(value, partition_pointer, R2C);
            break;
        case 30:
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, 30>
              <<<num_blocks, num_threads>>>(column_index, partition_pointer, R2C);
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_VT, ANONYMOUSLIB_UIT, 30>
              <<<num_blocks, num_threads>>>(value, partition_pointer, R2C);
            break;

        case 31:
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, 31>
              <<<num_blocks, num_threads>>>(column_index, partition_pointer, R2C);
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_VT, ANONYMOUSLIB_UIT, 31>
              <<<num_blocks, num_threads>>>(value, partition_pointer, R2C);
            break;
        case 32:
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, 32>
              <<<num_blocks, num_threads>>>(column_index, partition_pointer, R2C);
            aosoa_transpose_kernel_smem<ANONYMOUSLIB_VT, ANONYMOUSLIB_UIT, 32>
              <<<num_blocks, num_threads>>>(value, partition_pointer, R2C);
            break;
    }
    ...
    //    checkCudaErrors(cudaMemcpy(check_column_index, column_index, nnz * sizeof(ANONYMOUSLIB_IT),
    //    cudaMemcpyDeviceToHost));
    ...
    //    print_tile<ANONYMOUSLIB_IT>(&check_column_index[sigma * ANONYMOUSLIB_CSR5_OMEGA], sigma,
    //    ANONYMOUSLIB_CSR5_OMEGA);
