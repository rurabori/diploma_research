...
__inline__ __device__ iT binary_search_right_boundary_kernel(const iT* d_row_pointer, const iT key_input,
                                                             const iT size) {
    iT start = 0;
    iT stop = size - 1;
    iT median;
...
template<typename T>
__inline__ __device__ void sum_32(volatile T* s_sum, const int local_id) {
...
template<typename T>
__inline__ __device__ void scan_single(volatile T* s_scan, const int local_id, const int l) {
    T old_val, new_val;
    if (!local_id) {
        old_val = s_scan[0];
        s_scan[0] = 0;
...
// exclusive scan
template<typename T>
__inline__ __device__ void scan_32(volatile T* s_scan, const int local_id) {
    int ai, bi;
...
// inclusive scan
template<typename T>
__forceinline__ __device__ T scan_32_shfl(T x, const int local_id) {
    //    #pragma unroll
...
__forceinline__ __device__ double scan_32_shfl(double x) {
#pragma unroll
...
// exclusive scan
template<typename T>
__inline__ __device__ void scan_32_plus1(volatile T* s_scan, const int local_id) {
    int ai, bi;
    const int baseai = 2 * local_id + 1;
    const int basebi = baseai + 1;
    T temp;
...
template<typename T>
__inline__ __device__ T scan_plus1_shfl(volatile T* s_scan, const int local_id, T r_in, const int seg_num) {
    ...
    if (!seg_id) r_in = scan_32_shfl<T>(r_in, lane_id);

    if (local_id < seg_num) s_scan[local_id + 1] = r_in;
...
template<typename T>
__inline__ __device__ void scan_256_plus1(volatile T* s_scan) {
    int ai, bi;
    ...
    if (threadIdx.x == 0) {
        s_scan[255] += s_scan[127];
        s_scan[256] = s_scan[255];
        s_scan[255] = 0;
        temp = s_scan[127];
        s_scan[127] = 0;
        s_scan[255] += temp;
...
template<typename iT>
__forceinline__ __device__ void fetch_x(cudaTextureObject_t d_x_tex, const iT i, float* x) {
    *x = tex1Dfetch<float>(d_x_tex, i);
}

template<typename iT>
__forceinline__ __device__ void fetch_x(cudaTextureObject_t d_x_tex, const iT i, double* x) {
    int2 x_int2 = tex1Dfetch<int2>(d_x_tex, i);
    *x = __hiloint2double(x_int2.y, x_int2.x);
}

__global__ void warmup_kernel(int* d_scan) {
